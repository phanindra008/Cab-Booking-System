...
<HTML XMLns="http://www.w3.org/1999/xHTML"> 
  ...
  <H3>Already registered? <a href="Driverlogin.php">Login</a></H3>
...
<?php 
	...
	function ValidateEmail($emailid,$name1)
	{
		...
		if(strpos($emailid, '@') > 0 && strpos($emailid, '.') > strpos($emailid, '@')+1 && strpos(strrev($emailid), '.') > 0)
		{
			...
			$message = "Dear ". $name1 . ", welcome to use CabsOnline!";
		...
		//	$header = "From registration@cabsonline.com";
	...
	function RegisterUser($name,$password,$email,$phone)
	{
		...
		$SQLstring = "select count(*) from driver where Email = '".$email."'";
		...
		{
			$SQLstring = "Insert into driver values(null,'".$name."','".$password."','".$email."','".$phone."')";	
			$queryResult = @mysqli_query($DBConnect, $SQLstring)
