...
<HTML XMLns="http://www.w3.org/1999/xHTML"> 
  ...
  <form name="BookingForm" class="form2">
			<table>
			...
			<tr><td>Pick up address:</td><td>Door number:<input type="text" name="UnitNo"></td></tr>
			...
			<tr><td></td><td>City:<input type="text" name="Suburb"></td></tr>
			<tr><td>Destination :</td><td><input type="text" name="Dsuburb"></td></tr>
			...
			<tr><td>&nbsp&nbsp&nbsp<button type="submit">Book a Cab</button></td><td></td></tr>
		</table>
...
<?php 
	if(!(isset($_GET['Customer_Id'])) || empty($_GET['Customer_Id']))
	{
		...
		$query_string = "Customerlogin.php";
	...
	&& isset($_GET['Suburb']) && isset($_GET['Dsuburb']) && isset($_GET['pDate']) && isset($_GET['pTime']))
	{
			 ...
			 || empty($Suburb) || empty($Destination_Suburn) || empty($Pickup_Date) || empty($Pickup_Time))
		{
			...
			echo '<script>alert("Please provide details for all the fields")</script>';
			exit();
		}
		else
		{
	...
	function ValidBookingTime($Date,$Time)
	{
		...
		if(date_format($Date,'Y/m/j:H:i') < date('Y/m/j:H:i',strtotime('+2 hours')))
		{
			return false;
		}
		else
	...
	function CreateBooking($Customer_ID,$Passenger_Name,$Passenger_Phone,$Unit_No,$Street_No,$Street_Name,$Suburb,$Destination_Suburn,$Pickup_Date,$Pickup_Time)
	{
		...
		echo "<br><br><a href=home.php>Sign out</a>";
